...
def extract_purchase_bill_data(image_file):
    ...
    prompt = (
        ...
        "2. Supplier GSTIN / GST number (15-digit code e.g. 06AAACB8658E2ZO)\n"
        "3. Invoice number (bill number or reference number)\n"
        "4. Purchase/Invoice date (YYYY-MM-DD format if visible, or original text date converted to standard date)\n"
        "5. Payment mode ('Cash' or 'Credit' if visible, default to 'Credit')\n"
        "6. Line items (medicines/products table). For each item on EVERY row:\n"
